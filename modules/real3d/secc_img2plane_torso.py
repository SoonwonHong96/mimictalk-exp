...
class OSAvatarSECC_Img2plane_Torso(OSAvatarSECC_Img2plane):
    def __init__(self, hp=None, lora_args=None):
        ...
        self.large_sr = hp.get('large_sr', False)
        self.superresolution = SuperresolutionHybrid8XDC_Warp(channels=32, img_resolution=self.img_resolution, sr_num_fp16_res=self.sr_num_fp16_res, sr_antialias=True, lora_args=lora_args_sr, large_sr=self.large_sr, **self.sr_kwargs)
    ...
    def _forward_sr(self, rgb_image, feature_image, cond, ret, **synthesis_kwargs):
        ...
        sr_image, facev2v_ret = self.superresolution(rgb_image, feature_image, ones_ws, cond['ref_torso_img'], cond['bg_img'], ret['weights_img'], cond['segmap'], cond['kp_s'], cond['kp_d'], target_torso_mask=cond.get('target_torso_mask'), mouth_ref_img=cond.get('mouth_ref_img'), noise_mode=self.rendering_kwargs['superresolution_noise_mode'], **{k:synthesis_kwargs[k] for k in synthesis_kwargs.keys() if k != 'noise_mode'})
    ...
    def infer_synthesis_stage1(self, img, camera, cond=None, ret=None, update_emas=False, cache_backbone=False, use_cached_backbone=False, **synthesis_kwargs):
        ...
        ones_ws = torch.ones([feature_image.shape[0], 14, hparams['w_dim']], dtype=feature_image.dtype, device=feature_image.device)
        facev2v_ret = self.superresolution.infer_forward_stage1(rgb_image, feature_image, ones_ws, cond['ref_torso_img'], cond['bg_img'], ret['weights_img'], cond['segmap'], cond['kp_s'], cond['kp_d'], mouth_ref_img=cond.get('mouth_ref_img'), noise_mode=self.rendering_kwargs['superresolution_noise_mode'], **{k:synthesis_kwargs[k] for k in synthesis_kwargs.keys() if k != 'noise_mode'})
