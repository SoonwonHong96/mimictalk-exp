...
def smooth_features_xd_gaussian(
    ...
    Args:
        in_tensor (Tensor): shape [T, …]. Supported dims:
            [T, C], [T, C, H, W] or [T, C1, C2, H, W]
        kernel_size (int, optional): Length of 1-D filter. Default 5.
        strength (float, optional): 0 → no smoothing, 1 → full smoothing.
        kernel_type (str, optional): "box" | "gaussian".
        sigma (float, optional): Std-dev for gaussian (defaults to kernel_size / 3).

    Returns:
        Tensor: Smoothed features, same shape as input.
    """
    if not (0.0 <= strength <= 1.0):
        raise ValueError("strength must be in [0, 1]")

    if strength == 0.0 or kernel_size <= 1:
        return in_tensor  # fast exit
    ...
    if kernel_type == "gaussian":
        ...
        idx = torch.arange(kernel_size, dtype=torch.float32, device=device) - pad
        avg_kernel = torch.exp(-(idx**2) / (2 * sigma**2))
        avg_kernel = avg_kernel / avg_kernel.sum()
    elif kernel_type == "box":
        avg_kernel = torch.full((kernel_size,), 1.0 / kernel_size, device=device)
    else:
        raise ValueError("kernel_type must be 'box' or 'gaussian'")

    avg_kernel = avg_kernel.view(1, 1, kernel_size)  # [1,1,K]

    # -------- pad sequence (mirror) ----------------------------------------
    seq = torch.cat(
        [torch.flip(in_tensor[:pad], [0]), in_tensor, torch.flip(in_tensor[-pad:], [0])],
        dim=0,
    )  # [T+2*pad, …]

    # -------- reshape to [C',1,T] for conv1d --------------------------------
    ndim = in_tensor.ndim
    if ndim == 2:  # [T,C]
        seq_r = seq.permute(1, 0).unsqueeze(1)  # [C,1,T]
    elif ndim == 4:  # [T,C,H,W]
        seq_r = seq.permute(1, 2, 3, 0).reshape(-1, 1, t + 2 * pad)
    elif ndim == 5:  # [T,C1,C2,H,W]
        seq_r = seq.permute(1, 2, 3, 4, 0).reshape(-1, 1, t + 2 * pad)
    else:
        raise NotImplementedError(f"Unsupported ndim={ndim}")

    # -------- filtering -----------------------------------------------------
    smoothed_r = F.conv1d(seq_r, avg_kernel)  # same length as original T

    # -------- restore original layout --------------------------------------
    if ndim == 2:
        smoothed = smoothed_r.squeeze(1).permute(1, 0)
    elif ndim == 4:
        C, H, W = in_tensor.shape[1:4]
        smoothed = smoothed_r.squeeze(1).reshape(C, H, W, t).permute(3, 0, 1, 2)
    elif ndim == 5:
        C1, C2, H, W = in_tensor.shape[1:5]
        smoothed = (
            smoothed_r.squeeze(1)
            .reshape(C1, C2, H, W, t)
            .permute(4, 0, 1, 2, 3)
        )

    # -------- blend with original for strength control ----------------------
    if strength < 1.0:
        smoothed = strength * smoothed + (1.0 - strength) * in_tensor
